...
def process_image(name, debug, **options):
    ...
    for contour in contours:
        ...
        if validate_contour(contour, gray, aspect_ratio_range, area_range):
            ...
            if edge_density > 0.5:
                cv2.drawContours(input_image, [box], 0, (127,0,255),2, cv2.CHAIN_APPROX_SIMPLE)

            roi_plate = input_image[y1:y2, x1:x2]
            cv2.imwrite('C:\\TraViol\\plates\\{}.png'.format(i), roi_plate)
            cv2.waitKey(0)  

            i = i + 1

            ''' 
            for i, ctr in enumerate(box):
                # Get bounding box
                ...
                if w > 15 and h > 15:
                    cv2.imwrite('C:\\TraViol\\plates\\{}.png'.format(i), roi)
            '''
